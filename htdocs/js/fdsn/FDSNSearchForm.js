	...
	FDSNSearchForm.prototype = {
		...
		_fetchFieldData: function () {
			var form = this,
			    ...
			    callback = 'fdsn_callback_' + (CALLBACK_COUNT++),
			    cleanup = null,
			    cleanedUp = false;

			script.src = this.fieldDataUrl + '?callback=' + callback;

			cleanup = function () {
				if (cleanedUp) { return; }

				Util.removeEvent(script, 'load', cleanup);
				Util.removeEvent(script, 'error', cleanup);
			...
			window[callback] = function (data) {
				...
				// TODO :: Conform magnitude type to FDSN spec
				//form._enhanceField(data.magnitudetypes || [],
						//'magnitudetype', form.formatter.formatMagnitudeType, ['two-up']);
			...
			Util.addEvent(script, 'error', cleanup);
